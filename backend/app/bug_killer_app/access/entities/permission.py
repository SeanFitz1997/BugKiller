...
def assert_user_has_project_member_access(user_id: str, project: BkAppProject) -> None:
    if user_id != project.manager and user_id not in project.members:
        raise UnauthorizedProjectReadException(user_id, project.id)


def assert_user_has_project_manager_access(user_id: str, project: BkAppProject) -> None:
