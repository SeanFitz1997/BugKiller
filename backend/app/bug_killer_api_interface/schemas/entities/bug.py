...
from bug_killer_api_interface.test.test_doubles.default_values import mock_user_id, mock_bug_title, \
    mock_bug_description, mock_bug_tags
...
class BugResolution(BkBaseModel):
    ...
    resolved_on: ArrowType = Field(
        description='The date time that the bug was resolved on as an ISO 8601 date time string')
...
class Bug(BkBaseModel):
    ...
    title: str
    description: str
    created_on: ArrowType = Field(
        description='The date time that the bug was created on as an ISO 8601 date time string')
    last_updated_on: ArrowType = Field(
        description='The date time that the bug was last updated on as an ISO 8601 date time string')
